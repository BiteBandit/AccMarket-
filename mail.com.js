...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR MAIL.COM)
// ==========================================
async function fetchMailComInventory() {
    try {
        ...
        // CALIBRATED FOR MAIL.COM PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase().trim();
            return platform === 'mail.com';
        });

        applyFilters();
    } catch (err) {
        console.error("Mail.com Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (MAIL.COM COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Mail.com accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "mail.com").toLowerCase().trim();
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Mail.com account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Mail.com Listing";
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchMailComInventory(); // Refresh view state counts
            };
