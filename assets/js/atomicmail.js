...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR ATOMICMAIL)
// ==========================================
async function fetchAtomicmailInventory() {
    try {
        ...
        // CALIBRATED FOR ATOMICMAIL PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase().trim();
            return platform === 'atomicmail';
        });

        applyFilters();
    } catch (err) {
        console.error("Atomicmail Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (ATOMICMAIL COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Atomicmail accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "atomicmail").toLowerCase().trim();
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Atomicmail account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Atomicmail Listing";
    ...
    document.getElementById('modalConfirmBtn').onclick = async () => {
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            // Append business appreciation note
            rawTxtContent += `=========================================\n`;
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchAtomicmailInventory(); // Refresh view state counts
            };
...
async function init() {
    ...
    await fetchAtomicmailInventory(); 
