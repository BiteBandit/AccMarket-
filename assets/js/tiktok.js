...
// 2. Main Fetch Function (UPDATED FOR TIKTOK INVENTORY FILTERING)
async function fetchTikTokInventory() {
    try {
    ...
    .in('id', userIds);
        ...
        activeAccounts = inventory.map(row => {
            ...
            return platform === 'tiktok'; // 🌟 Only target TikTok items
        });
...
// 3. Pure Synchronous Render Function (UPDATED WITH TIKTOK IMAGE URLS)
    ...
    grid.innerHTML = accounts.map(row => {
        ...
        return `
            <div class="card">
                ...
                <div class="card-img">
                    <img src="https://accmarket.name.ng/images/tiktok.png" alt="TT">
                </div>
...
window.openDetails = async (id) => {
    ...
    body.innerHTML = `
        <img src="${acc.screenshot_url || 'https://accmarket.name.ng/images/tiktok.png'}" style="width:100%; border-radius:12px; margin-bottom:15px; border:1px solid var(--border);">
...
// Escrow Processing Transaction Pipeline (UPDATED FOR TIKTOK ENGINE LOGS)
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for TikTok account: @${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "TikTok Listing";
        const chatRoomUrl = `https://accmarket.name.ng/chats?id=${conv.id}`;
        
        const purchaseAlertMsg = `@${currentBuyerName} purchased your TikTok account @${targetAccountName}! Access your chat workspace here to deliver logs: ${chatRoomUrl}`;
