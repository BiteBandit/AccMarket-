...
// 2. Main Fetch Function (UPDATED FOR LINKEDIN INVENTORY FILTERING)
async function fetchLinkedInInventory() {
    try {
    ...
    .in('id', userIds);
        ...
        activeAccounts = inventory.map(row => {
            ...
            return platform === 'linkedin'; // 🌟 Only target LinkedIn items
        });
...
// 3. Pure Synchronous Render Function (UPDATED WITH LINKEDIN IMAGE URLS)
    ...
    grid.innerHTML = accounts.map(row => {
        ...
        return `
            <div class="card">
                ...
                <div class="card-img">
                    <img src="https://accmarket.name.ng/images/linkedin.png" alt="LinkedIn">
                </div>
                <p class="title">${meta.username || 'Unknown Account'}</p>
                <div class="meta">
...
window.openDetails = async (id) => {
    ...
    body.innerHTML = `
        <img src="${acc.screenshot_url || 'https://accmarket.name.ng/images/linkedin.png'}" style="width:100%; border-radius:12px; margin-bottom:15px; border:1px solid var(--border);">
        <div class="detail-item"><span class="detail-label">Account Name/ID</span><span class="detail-value">${meta.username || 'Unknown'}</span></div>
        ...
        <div class="detail-item"><span class="detail-label">Connections/Followers</span><span class="detail-value">${(meta.followers || 0).toLocaleString()}</span></div>
...
// Escrow Processing Transaction Pipeline (UPDATED FOR LINKEDIN ENGINE LOGS)
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for LinkedIn account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "LinkedIn Listing";
        const chatRoomUrl = `https://accmarket.name.ng/chats?id=${conv.id}`;
        
        const purchaseAlertMsg = `@${currentBuyerName} purchased your LinkedIn account ${targetAccountName}! Access your chat workspace here to deliver logs: ${chatRoomUrl}`;
