...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR HOTMAIL)
// ==========================================
async function fetchHotmailInventory() {
    try {
        ...
        // CALIBRATED FOR HOTMAIL PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase();
            return platform === 'hotmail';
        });

        applyFilters();
    } catch (err) {
        console.error("Hotmail Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (HOTMAIL COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Hotmail accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "hotmail").toLowerCase().trim();
        ...
        if (row.is_system) {
            ...
            return `
                <div class="card system-stock-card" style="border: 1.5px dashed #6001D2; background: #fbfbfe; opacity: ${isOutOfStock ? '0.7' : '1'}; position: relative; padding: 14px; min-height: 290px !important; display: flex; flex-direction: column; justify-content: space-between; gap: 10px; box-sizing: border-box;">
                    <div style="display: flex; flex-direction: column; justify-content: space-between; flex-grow: 1;">
                        <div>
                            <div class="card-tag system-tag" style="background: ${isOutOfStock ? '#64748b' : '#6001D2'}; color: #fff; font-weight: 700; font-size: 0.65rem; border-radius: 4px; padding: 2px 6px; position: absolute; top: 10px; left: 10px; display: inline-flex; align-items: center; gap: 3px;">
                    ...
                    <div style="margin-top: auto; display: flex; flex-direction: column; gap: 6px;">
                        <div class="price-row" style="text-align: center;">
                            ...
                            <span class="price" style="color:#6001D2; font-weight:800; font-size: 1.1rem;">₦${parseFloat(meta.price || 0).toLocaleString()}</span>
                        </div>

                        <div class="card-action-container" style="margin: 0; padding: 0;">
                            <button class="btn purchase-btn" onclick="${isOutOfStock ? '' : `initiateBulkSystemPurchase('${row.id}')`}" ${isOutOfStock ? 'disabled' : ''} style="background: ${isOutOfStock ? '#94a3b8' : '#6001D2'}; width:100%; padding: 9px; font-size: 0.8rem; font-weight: 700; border-radius: 6px;">
            ...
            <div class="card" style="padding: 14px; min-height: 290px !important; display: flex; flex-direction: column; justify-content: space-between; gap: 10px; position: relative; box-sizing: border-box;">
                <div style="display: flex; flex-direction: column; justify-content: space-between; flex-grow: 1;">
                    ...
                    <div class="meta" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px 6px; font-size: 0.72rem; color: #64748b; margin: auto 0 4px 0; padding: 0 4px;">
                        <span style="display: flex; align-items: center; gap: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><i class="fa-solid fa-earth-africa" style="color: #6001D2;"></i> ${meta.region || 'Global'}</span>
                        <span style="display: flex; align-items: center; gap: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><i class="fa-solid fa-envelope" style="color: #6001D2;"></i> ${(meta.followers || 0)} Mails</span>
                        <span style="display: flex; align-items: center; gap: 3px;"><i class="fa-regular fa-calendar" style="color: #6001D2;"></i> ${displayYear}</span>
                        <span onclick="openDetails('${row.id}')" style="display: flex; align-items: center; gap: 3px; color: #6001D2; font-weight: 700; cursor: pointer; white-space: nowrap;"><i class="fa-solid fa-circle-info"></i> Details</span>
                    </div>
                </div>
...
window.openDetails = async (id) => {
    ...
    body.innerHTML = `
        ...
        <div class="detail-item"><span class="detail-label">Verification Token</span><span class="detail-value" style="color:#6001D2; font-weight:bold;">${meta.verification_code || 'N/A'}</span></div>
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        const { data: buyer, error: balanceError } = await supabase
            .from('profiles').select('username, full_name, balance').eq('id', buyerId).single();
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Hotmail account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Hotmail Listing";
