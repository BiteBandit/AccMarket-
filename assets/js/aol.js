...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR AOL)
// ==========================================
async function fetchAolInventory() {
    try {
        ...
        // CALIBRATED FOR AOL PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase().trim();
            return platform === 'aol' || platform === 'aol.com';
        });

        applyFilters();
    } catch (err) {
        console.error("AOL Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (AOL COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified AOL accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "aol").toLowerCase().trim();
        if (platformKey === "aol.com") platformKey = "aol";
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for AOL account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "AOL Listing";
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchAolInventory(); // Refresh view state counts
            };
