...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR GMX)
// ==========================================
async function fetchGMXInventory() {
    try {
        ...
        // CALIBRATED FOR GMX PLATFORM DATA ONLY
        ...
        console.error("GMX Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (GMX COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified GMX accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "gmx").toLowerCase().trim();
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for GMX account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "GMX Listing";
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchGMXInventory(); // Refresh view state counts
            };
