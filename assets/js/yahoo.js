...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR YAHOO)
// ==========================================
async function fetchYahooInventory() {
    try {
        ...
        // CALIBRATED FOR YAHOO PLATFORM DATA ONLY
        ...
        console.error("Yahoo Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (YAHOO COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Yahoo accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "yahoo").toLowerCase().trim();
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Yahoo account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Yahoo Listing";
