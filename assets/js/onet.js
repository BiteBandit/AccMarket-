...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR ONET)
// ==========================================
async function fetchOnetInventory() {
    try {
        ...
        // CALIBRATED FOR ONET PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase().trim();
            return platform === 'onet' || platform === 'onet.pl';
        });

        applyFilters();
    } catch (err) {
        console.error("Onet Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (ONET COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Onet accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "onet").toLowerCase().trim();
        if (platformKey === "onet.pl") platformKey = "onet";
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Onet account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Onet Listing";
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchOnetInventory(); // Refresh view state counts
            };
