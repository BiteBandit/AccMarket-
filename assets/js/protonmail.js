...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR PROTONMAIL)
// ==========================================
async function fetchProtonMailInventory() {
    try {
        ...
        // CALIBRATED FOR PROTONMAIL PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase();
            return platform === 'protonmail';
        });

        applyFilters();
    } catch (err) {
        console.error("ProtonMail Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (PROTONMAIL COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified ProtonMail accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "protonmail").toLowerCase().trim();
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for ProtonMail account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "ProtonMail Listing";
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchProtonMailInventory(); // Refresh view state counts
            };
...
async function init() {
    ...
    await fetchProtonMailInventory(); 
