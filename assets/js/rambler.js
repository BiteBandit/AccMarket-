...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR RAMBLER)
// ==========================================
async function fetchRamblerInventory() {
    try {
        const urlParams = new URLSearchParams(window.location.search);
        const targetId = urlParams.get('id');

        // A. Fetch individual seller escrow listings
        ...
        // B. Fetch system bulk stock rows
        ...
        // Process standard escrow listings
        ...
        // Process unified system bulk stocks
        ...
        // CALIBRATED FOR RAMBLER PLATFORM ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase();
            return platform === 'rambler';
        });

        applyFilters();
    } catch (err) {
        console.error("Rambler Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (RAMBLER COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Rambler accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "rambler").toLowerCase().trim();
                ...
                <div class="card system-stock-card" style="border: 1.5px dashed #4735ea; background: #fbfbfe; opacity: ${isOutOfStock ? '0.7' : '1'}; position: relative; padding: 14px; min-height: 290px !important; display: flex; flex-direction: column; justify-content: space-between; gap: 10px; box-sizing: border-box;">
                    ...
                    <div style="margin-top: auto; display: flex; flex-direction: column; gap: 6px;">
                        <div class="price-row" style="text-align: center;">
                            ...
                            <span class="price" style="color:var(--primary); font-weight:800; font-size: 1.1rem;">₦${parseFloat(meta.price || 0).toLocaleString()}</span>
                        </div>
            ...
            <div class="card" style="padding: 14px; min-height: 290px !important; display: flex; flex-direction: column; justify-content: space-between; gap: 10px; position: relative; box-sizing: border-box;">
                <div style="display: flex; flex-direction: column; justify-content: space-between; flex-grow: 1;">
                    ...
                    <div class="meta" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 8px 6px; font-size: 0.72rem; color: #64748b; margin: auto 0 4px 0; padding: 0 4px;">
                        <span style="display: flex; align-items: center; gap: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><i class="fa-solid fa-earth-africa" style="color: var(--primary);"></i> ${meta.region || 'Global'}</span>
                        <span style="display: flex; align-items: center; gap: 3px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><i class="fa-solid fa-envelope" style="color: var(--primary);"></i> ${(meta.followers || 0)} Mails</span>
                        <span style="display: flex; align-items: center; gap: 3px;"><i class="fa-regular fa-calendar" style="color: var(--primary);"></i> ${displayYear}</span>
                        <span onclick="openDetails('${row.id}')" style="display: flex; align-items: center; gap: 3px; color: var(--primary); font-weight: 700; cursor: pointer; white-space: nowrap;"><i class="fa-solid fa-circle-info"></i> Details</span>
                    </div>
                </div>
...
window.openDetails = async (id) => {
    ...
    body.innerHTML = `
        ...
        <div class="detail-item"><span class="detail-label">Verification Token</span><span class="detail-value" style="color:var(--primary); font-weight:bold;">${meta.verification_code || 'N/A'}</span></div>
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Rambler account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Rambler Listing";
    ...
    document.getElementById('modalConfirmBtn').onclick = async () => {
        ...
        try {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchRamblerInventory(); // Refresh view state counts
            };
