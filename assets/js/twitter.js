...
// 2. Main Fetch Function (UPDATED FOR TWITTER INVENTORY FILTERING)
async function fetchTwitterInventory() {
    try {
    ...
    .in('id', userIds);
        ...
        activeAccounts = inventory.map(row => {
            ...
            return platform === 'twitter' || platform === 'x'; // 🌟 Targets Twitter or X items
        });
...
// 3. Pure Synchronous Render Function (UPDATED WITH TWITTER IMAGE URLS)
    ...
    grid.innerHTML = accounts.map(row => {
        ...
        return `
            <div class="card">
                ...
                <div class="card-img">
                    <img src="https://accmarket.name.ng/images/twitter.png" alt="Twitter">
                </div>
...
window.openDetails = async (id) => {
    ...
    body.innerHTML = `
        <img src="${acc.screenshot_url || 'https://accmarket.name.ng/images/twitter.png'}" style="width:100%; border-radius:12px; margin-bottom:15px; border:1px solid var(--border);">
...
// Escrow Processing Transaction Pipeline (UPDATED FOR TWITTER ENGINE LOGS)
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Twitter account: @${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Twitter Listing";
        const chatRoomUrl = `https://accmarket.name.ng/chats?id=${conv.id}`;
        
        const purchaseAlertMsg = `@${currentBuyerName} purchased your Twitter account @${targetAccountName}! Access your chat workspace here to deliver logs: ${chatRoomUrl}`;
