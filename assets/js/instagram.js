...
// 2. Main Fetch Function (UPDATED FOR INSTAGRAM INVENTORY FILTERING)
async function fetchInstagramInventory() {
    try {
    ...
    .in('id', userIds);
        ...
        activeAccounts = inventory.map(row => {
            ...
            return platform === 'instagram'; // 🌟 Only target Instagram items
        });
...
// 3. Pure Synchronous Render Function (UPDATED WITH INSTAGRAM IMAGE URLS)
    ...
    grid.innerHTML = accounts.map(row => {
        ...
        return `
            <div class="card">
                ...
                <div class="card-img">
                    <img src="https://accmarket.name.ng/images/instagram.png" alt="IG">
                </div>
...
window.openDetails = async (id) => {
    ...
    body.innerHTML = `
        <img src="${acc.screenshot_url || 'https://accmarket.name.ng/images/instagram.png'}" style="width:100%; border-radius:12px; margin-bottom:15px; border:1px solid var(--border);">
...
// Escrow Processing Transaction Pipeline (UPDATED FOR INSTAGRAM ENGINE LOGS)
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Instagram account: @${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Instagram Listing";
        const chatRoomUrl = `https://accmarket.name.ng/chats?id=${conv.id}`;
        
        const purchaseAlertMsg = `@${currentBuyerName} purchased your Instagram account @${targetAccountName}! Access your chat workspace here to deliver logs: ${chatRoomUrl}`;
...
document.addEventListener("click", async (e) => {
  if (e.target.closest(".logout")) {
    ...
    try {
      ...
      // Redirect the interface directly to the auth folder route
      window.location.href = "../auth";
    } catch (err) {
