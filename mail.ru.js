...
// ==========================================
// 4. CORE DATA SOURCE INVENTORY FETCH (FILTERED FOR MAIL.RU)
// ==========================================
async function fetchMailRuInventory() {
    try {
        ...
        // CALIBRATED FOR MAIL.RU PLATFORM DATA ONLY
        activeAccounts = [...processedSystem, ...processedUsers].filter(row => {
            const platform = row.data?.platform?.toLowerCase().replace('_', '.');
            return platform === 'mail.ru';
        });

        applyFilters();
    } catch (err) {
        console.error("Mail.ru Engine Fetch Failure:", err.message);
    }
}

// ==========================================
// 5. DOM GRID RENDERING LOGIC (MAIL.RU COMPLIANT)
// ==========================================
function renderGrid(accounts) {
    ...
    if (accounts.length === 0) {
        grid.innerHTML = `<p style="grid-column: 1/-1; text-align: center; color: var(--text-muted); padding: 40px;">No verified Mail.ru accounts found matching your criteria.</p>`;
        return;
    }
    ...
    grid.innerHTML = accounts.map(row => {
        const meta = row.data;
        let platformKey = String(meta.platform || "mail.ru").toLowerCase().trim();
...
async function processTransaction(account, totalWithFee, originalPrice, buyerId) {
    try {
        ...
        await supabase.from('wallet').insert([{
            ...
            note: `Payment for Mail.ru account: ${account.data.username || 'Unknown'}`,
            status: 'success',
        ...
        const targetAccountName = account.data?.username || "Mail.ru Listing";
        ...
        try {
          ...
          itemsToDeliver.forEach((account, index) => {
            ...
            document.getElementById('modalCloseDeliveryBtn').onclick = () => {
                modalOverlay.remove();
                fetchMailRuInventory(); // Refresh view state counts
            };
